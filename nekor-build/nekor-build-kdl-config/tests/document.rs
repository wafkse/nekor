    ...
    fn parse_single_element_list() {
        ...
        if let Ok(document) = result {
            assert!(
                document
                    .get("values")
                    .is_some_and(|node| { matches!(node.kind(), NodeKind::List(values) if values.len() == 1) })
            );
        }
    }
    ...
    fn reject_duplicate_terminal_merge() {
        ...
        if let (Ok(left), Ok(right)) = (left, right) {
            ...
            }) = result
            {
                ...
                assert_eq!(existing.source().path().map(|path| path.as_str()), Some("left.kdl"));
                assert_eq!(incoming.source().path().map(|path| path.as_str()), Some("right.kdl"));
            }
        }
    }
